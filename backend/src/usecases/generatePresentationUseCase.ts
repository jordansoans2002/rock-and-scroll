...
export class GeneratePresentationUseCase {
    ...
    async execute(payload: CreatePresentationRequest): Promise<Buffer> {
        ...
        for(const song of payload.songs) {
            ...
            const partsByStanza = this.splitByStanzas({song.text1, song.text2}, song.settings.stanzas)
            ...
            for (const part of partsBySybmol) {
                ...
                for (const block of partsByLine) {
                    this.generator.renderLyricsSlide(presentation.addSlide(), song.settings, block, payload.presentationSettings);
                }
            }
        }    
